...
#define GF_GAIN_ADJUST_COEFF	5
#define TSMC_GAIN_ADJUST_COEFF	60
#endif
...
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
				sys_gain_coeff = div_s64(sys_gain_coeff, 100)*GF_GAIN_ADJUST_COEFF;
#endif
			}
			break;
		case COMP_ID_TSMC:
...
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
				sys_gain_coeff = div_s64(sys_gain_coeff, 100)*TSMC_GAIN_ADJUST_COEFF;
#endif
			}
			break;
		}
...
static int32_t qpnp_iadc_comp_info(struct qpnp_iadc_chip *iadc)
{
	...
	pr_info("fab id = %u, revision_dig_major = %u, revision_ana_minor = %u sys gain = %u, external_rsense = %d\n",
			iadc->iadc_comp.id,
			iadc->iadc_comp.revision_dig_major,
			iadc->iadc_comp.revision_ana_minor,
			iadc->iadc_comp.sys_gain,
