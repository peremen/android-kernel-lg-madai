...
#define REG_BL_OFF	0
#define REG_0_BL	118
#define REG_20_BL	200
#define REG_40_BL	528
#define REG_60_BL	1114
#define REG_80_BL	1922
#define REG_MAX_BL	3114
#endif
//            
				      ...
				      enum led_brightness value)
{
	...
	if(value >= UI_BL_OFF && value <= UI_0_BL)
		bl_lvl = (value - UI_BL_OFF) * (REG_0_BL - REG_BL_OFF) / (UI_0_BL - UI_BL_OFF) + REG_BL_OFF;
	else if(value >= UI_0_BL && value <= UI_20_BL)
		bl_lvl = (value - UI_0_BL) * (REG_20_BL - REG_0_BL) / (UI_20_BL - UI_0_BL) + REG_0_BL;
	else if(value >UI_20_BL && value <= UI_40_BL)
		bl_lvl = (value - UI_20_BL) * (REG_40_BL - REG_20_BL) / (UI_40_BL - UI_20_BL) + REG_20_BL;
	else if(value >UI_40_BL && value <= UI_60_BL)
		bl_lvl = (value - UI_40_BL) * (REG_60_BL - REG_40_BL) / (UI_60_BL - UI_40_BL) + REG_40_BL;
	else if(value >UI_60_BL && value <= UI_80_BL)
		bl_lvl = (value - UI_60_BL) * (REG_80_BL - REG_60_BL) / (UI_80_BL - UI_60_BL) + REG_60_BL;
	else if(value >UI_80_BL && value <= UI_MAX_BL)
		bl_lvl = (value - UI_80_BL) * (REG_MAX_BL - REG_80_BL) / (UI_MAX_BL - UI_80_BL) + REG_80_BL;
...
//                                                                                                                                                                                   
	...
	if(lge_get_boot_mode() == LGE_BOOT_MODE_QEM_130K) {
		...
		if ((pdata) && (pdata->set_backlight)) {
			...
			pr_info("%s : force to set bl_level[%d] regardless of bl_updated \n", __func__, temp);
		}
