...
//                                                        
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
static int mdss_dsi_unblank_forced(struct mdss_panel_data *pdata)
{
	int ret = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	if(ctrl_pdata->on_forced) {
		ret = ctrl_pdata->on_forced(pdata);
		if (ret) {
		...
		pr_err("%s: Invalid ctrl_pdata->on_forced \n", __func__);
	}

	return ret;
}

static int mdss_dsi_tp_control(struct mdss_panel_data *pdata, int enable)
{
	int ret = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	if(ctrl_pdata->tp_ctrl) {
		ret = ctrl_pdata->tp_ctrl(pdata, enable);
		if (ret) {
			pr_err("%s: unable to initialize the panel\n",
							__func__);
			return ret;
...
//            
	...
	case TOUCH_EVENT_RESET_START:
		if(hw_rev == HW_REV_A || hw_rev == HW_REV_B) {
			pr_err("[psh][%s] TOUCH_EVENT_RESET_START received \n", __func__);
			unblank_required = true;
		}
		break;
	case TOUCH_EVENT_REGISTER_DONE:
		...
		break;
	case LCD_EVENT_TP_DISABLE:
		if(hw_rev >= HW_REV_A) {
			mdss_dsi_tp_control(cp_pdata, 0);
		}
		break;
	case LCD_EVENT_TP_ENABLE:
		if(hw_rev >= HW_REV_A) {
			mdss_dsi_tp_control(cp_pdata, 1);
		}
		break;
	default:
		break;
...
//                                                                                                
		...
		if(hw_rev >= HW_REV_A) {
			if(touch_driver_registered)
				touch_notifier_call_chain(LCD_EVENT_ON_START, NULL);
		}
#endif
//            
...
//                                                                                                         
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
	case MDSS_EVENT_UNBLANK_FORCED:
		if(hw_rev == HW_REV_A || hw_rev == HW_REV_B) {
			pr_err("[psh][%s] MDSS_EVENT_UNBLANK_FORCED handled \n", __func__);
			rc = mdss_dsi_unblank_forced(pdata);
		}
		break;
#endif
//            
...
//                                                                                                
		...
		if(hw_rev >= HW_REV_A) {
			if(touch_driver_registered)
				touch_notifier_call_chain(LCD_EVENT_ON_END, NULL);
		}
#endif
//            
		break;
...
//                                                                                                
		...
		if(hw_rev >= HW_REV_A) {
			if(touch_driver_registered)
				touch_notifier_call_chain(LCD_EVENT_OFF_START, NULL);
		}
#endif
//            
...
//                                                                                                
		...
		if(hw_rev >= HW_REV_A) {
			if(touch_driver_registered)
				touch_notifier_call_chain(LCD_EVENT_OFF_END, NULL);
		}
#endif
//            
		break;
...
//                                                                                                         
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
	unblank_required = false;
	touch_driver_registered = false;

	if(!(lge_get_boot_mode() == LGE_BOOT_MODE_QEM_130K
		|| lge_get_boot_mode() == LGE_BOOT_MODE_QEM_910K || lge_get_boot_mode() == LGE_BOOT_MODE_PIF_910K)) {
...
//                                               
	...
	if(hw_rev == HW_REV_0) {
		...
		ctrl_pdata->disp_lcd_io_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-lcd-io-gpio", 0);

		if (!gpio_is_valid(ctrl_pdata->disp_lcd_io_gpio))
			pr_err("%s:%d, LCD IO gpio not specified\n",
							__func__, __LINE__);
	}
