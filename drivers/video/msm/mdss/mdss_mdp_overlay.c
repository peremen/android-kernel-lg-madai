...
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
	if(unblank_required) {
		mdss_mdp_ctl_intf_event(ctl, MDSS_EVENT_UNBLANK_FORCED, NULL);
		unblank_required = false;
		pr_err("[psh][%s] MDSS_EVENT_UNBLANK_FORCED sent, unblank_required[%d] \n", __func__, unblank_required);
	}
#endif
//            
