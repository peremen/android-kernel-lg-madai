...
//                                                
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
...
static struct dsi_panel_cmds lge_display_tp_on_cmds;
static struct dsi_panel_cmds lge_display_tp_off_cmds;
#endif
//            
...
//                                                 
	...
	//for dsv-vsn
	if(hw_rev == HW_REV_0) {
		//same gpio as vsp - does not need to be controlled
	} else {
	...
	//for lcd-io
	if(hw_rev == HW_REV_0) {
		//ldo8 is used for lcd-io
	} else if(hw_rev == HW_REV_A || hw_rev == HW_REV_B) {
		//ldo6 is used for lcd-io
	} else {
		if (gpio_is_valid(ctrl_pdata->disp_lcd_io_gpio)) {
			rc = gpio_request(ctrl_pdata->disp_lcd_io_gpio,
							"disp_lcd_io_gpio");
			if (rc) {
				pr_err("request disp_lcd_io_gpio gpio failed, rc=%d\n",
					       rc);
				goto disp_en_gpio_err;
			}
		}
...
//                                                 
	...
	//for lcd-io
	if(hw_rev == HW_REV_0) {
		//nothing allocated for lcd-io
	} else if(hw_rev == HW_REV_A || hw_rev == HW_REV_B) {
		//nothing allocated for lcd-io
	} else {
		if (gpio_is_valid(ctrl_pdata->disp_lcd_io_gpio))
			gpio_free(ctrl_pdata->disp_lcd_io_gpio);
	}
#endif
//            
disp_en_gpio_err:
...
//            
	...
	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_debug("%s:%d, enable gpio not configured\n",
			   __func__, __LINE__);
	}
...
//                                                                    
		...
		if (hw_rev == HW_REV_0){
			...
			if (hw_rev >= HW_REV_C){ //since RevC., lcd-io is controlled by gpio
				if (gpio_is_valid(ctrl_pdata->disp_lcd_io_gpio)) {
					gpio_set_value((ctrl_pdata->disp_lcd_io_gpio), 1);
				}
			}
			usleep(10*1000);
			if (gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
...
//            
		...
		if (hw_rev == HW_REV_0){
			...
			if (hw_rev >= HW_REV_C){ //since RevC., lcd-io is controlled by gpio
				if (gpio_is_valid(ctrl_pdata->disp_lcd_io_gpio)) {
					gpio_set_value((ctrl_pdata->disp_lcd_io_gpio), 0);
					if(gpio_request_done){
						gpio_free(ctrl_pdata->disp_lcd_io_gpio);
					}
				}
...
//                                                        
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
static int mdss_dsi_panel_on_forced(struct mdss_panel_data *pdata)
{
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	...
	if (ctrl->on_cmds.cmd_cnt){
		...
		pr_err("[psh][%s] 1st cmd sent *** \n", __func__);
	}
	if (lge_display_on_cmds.cmd_cnt) {
		mdss_dsi_panel_cmds_send(ctrl, &lge_display_on_cmds);
		pr_err("[psh][%s] 2nd cmd sent *** \n", __func__);
	}

	return 0;
}

static int mdss_dsi_panel_tp_control(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	...
	if(enable) {
		if (lge_display_tp_on_cmds.cmd_cnt){
			mdss_dsi_panel_cmds_send(ctrl, &lge_display_tp_on_cmds);
			pr_err("[LCD] lge_display_tp_on_cmds sent \n");
		}
	} else {
		if (lge_display_tp_off_cmds.cmd_cnt){
			mdss_dsi_panel_cmds_send(ctrl, &lge_display_tp_off_cmds);
		}
	}
...
//            
	...
	if (lge_display_on_cmds.cmd_cnt) {
		mdss_dsi_panel_cmds_send(ctrl, &lge_display_on_cmds);
		pr_err("[LCD] lge_display_on_cmds sent \n");
	}
#else //qct original
	if (ctrl->on_cmds.cmd_cnt)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds);
...
//                                                                                                   
	...
	if(hw_rev < HW_REV_1_0) {
		...
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
			"qcom,mdss-dsi-on-command-with-tpcm", "qcom,mdss-dsi-on-command-state");
	}
#else //qct original
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-command", "qcom,mdss-dsi-on-command-state");
...
//                                               
	...
	mdss_dsi_parse_dcs_cmds(np, &lge_display_tp_on_cmds,
		"lge,tp-controller-enable", "qcom,mdss-dsi-on-command-state");

	mdss_dsi_parse_dcs_cmds(np, &lge_display_tp_off_cmds,
		"lge,tp-controller-disable", "qcom,mdss-dsi-off-command-state");
#endif
//            
...
//                                                        
#if defined(CONFIG_LGE_MIPI_JDI_PANEL)
	ctrl_pdata->on_forced = mdss_dsi_panel_on_forced; //for preventing lcd black-out by touch hw reset
	ctrl_pdata->tp_ctrl = mdss_dsi_panel_tp_control; //for preventing display noise issue by touch ic's pe operation
#endif
//            
