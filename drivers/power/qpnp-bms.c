...
struct qpnp_bms_chip {
	...
	/* platform data */
	int				r_sense_uohm;
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
	int				charging_r_sense_uohm;
	int				discharging_r_sense_uohm;
#endif
	...
	if (is_battery_full(chip)) {
		...
		*vbat_uv = (int)v_result.physical;
		...
		if(chip->battery_status == POWER_SUPPLY_STATUS_CHARGING) {
			i_result.result_ua = div_s64((int64_t)i_result.result_ua*chip->discharging_r_sense_uohm,
				chip->charging_r_sense_uohm);
		}
#endif
		/*
...
#define CC_ACCURACY_LOW_SOC_THRESHOLD      96
				...
				int batt_temp)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
        ...
        pr_info("PON_OVC_UV = %d, cc = %llx, boot_mode = %d\n",
    ...
    if(is_battery_full(chip) && 
		...
		chip->software_shdw_cc_uah = 0;
		chip->last_cc_uah = INT_MIN;
        pr_info("update new estimated ocv : chip->last_ocv_uv = %d\n", chip->last_ocv_uv);
    }
#endif
...
//	int prev_rescale_soc = chip->rescale_soc;
//	int prev_soc = chip->last_soc;
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
//	if((prev_rescale_soc != chip->rescale_soc) || (soc != prev_soc)) {
//		pr_info("Reported_SoC(rescale) : %d, prev_rescale_soc: %d, raw_soc: %d, prev_raw_soc: %d\n",
//			chip->rescale_soc, prev_rescale_soc, soc, prev_soc);
//	}
...
/* At high temperature, device should be woke up as soon as possible for safety. */
	...
	pr_info("Rescale_SOC = %d, New_cal_SOC = %d, SOC = %d, batt_temp = %d, rbatt = %d, fcc_uah = %d, ocv_charge_uah = %d, uuc_uah = %d, cc_uah = %d, iavg_ua = %d\n",
		chip->rescale_soc, new_calculated_soc, soc, batt_temp, params.rbatt_mohm, params.fcc_uah, params.ocv_charge_uah, params.uuc_uah, params.cc_uah, params.iavg_ua);
...
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
		if(chip->battery_status == POWER_SUPPLY_STATUS_CHARGING) {
			chip->r_sense_uohm= chip->charging_r_sense_uohm;
		}
		else {
				chip->r_sense_uohm = chip->discharging_r_sense_uohm;
		}
		pr_info("rsense_ohm: %d\n", chip->r_sense_uohm);
#endif
...
static void qpnp_bms_external_power_changed(struct power_supply *psy)
{
	struct qpnp_bms_chip *chip = container_of(psy, struct qpnp_bms_chip,
								bms_psy);
#ifndef CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	switch ( battery_id ){
		...
		case BATT_ID_DS2704_C :
			batt_data = &LGE_BL_59UH_2440mAh_TOCAD_MADAI_data;
			pr_info("[BATTERY PROFILE] Using battery profile - TOCAD_MADAI_2440mAh for id(%d)\n",battery_id);
			break;
		...
		default :
			batt_data = &LGE_BL_59UH_2440mAh_TOCAD_MADAI_data;
			pr_err("[BATTERY PROFILE] No battery ID matching\nUsing default profile - TOCAD_MADAI_2440mAh for id(%d)\n",battery_id);
			break;
	}
#else
...
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
	SPMI_PROP_READ(charging_r_sense_uohm, "charging-r-sense-uohm", rc);
	chip->discharging_r_sense_uohm = chip->r_sense_uohm;
#endif
...
#ifdef CONFIG_LGE_PM_ADJUST_BMS_CHARGING_CURRENT //temprary apply this for avoid charging SoC error
	pr_info("[BMS_DEBUG][bms_read_properties] dts data: r_sense_uohm:%d, charging_r_sense_uohm:%d, v_cutoff_uv:%d, max_v:%d\n",
			chip->r_sense_uohm, chip->charging_r_sense_uohm, chip->v_cutoff_uv,
			chip->max_voltage_uv);
#else
