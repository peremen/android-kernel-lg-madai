...
#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
	struct delayed_work 	battery_remove_check_work;
	bool					batt_is_not_present;
#endif
...
static void lge_pm_set_ext_ovp(int ext_ovp_gpio, int on)
{
	if (gpio_get_value(ext_ovp_gpio) != on) {
		pr_info("=== Ext. OVP : %s ===\n", on ? "ON(DCP Cable)" : "OFF(SDP Cable)");
...
#ifdef CONFIG_LGE_PM
int external_qpnp_chg_is_usb_chg_plugged_in(void)
{
...
qpnp_arb_stop_work(struct work_struct *work)
{
	...
	pr_info("qpnp_arb_stop_work: chg_done: %d\n", chip->chg_done);
...
#ifndef	CONFIG_MACH_MSM8926_MADAI
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	pr_info("chg_gone triggered: usb_present:%d, usb_sts:0x%x\n",
		qpnp_chg_is_usb_chg_plugged_in(chip), usb_sts);
...
static void qpnp_chg_usb_valid_work(struct work_struct *work)
{
	...
	pr_err("usb_present: %d host_mode: %d\n", usb_present, host_mode);
...
qpnp_chg_bat_if_batt_pres_irq_handler(int irq, void *_chip)
{
	...
	if (chip->batt_present ^ batt_present) {
		...
		if(chip->batt_present) {
			schedule_delayed_work(&chip->battery_remove_check_work,
				msecs_to_jiffies(0));
		}
		else {
			if(chip->usb_present) {
	...
	if (chip->fastchg_on ^ fastchg_on) {
		...
		if (fastchg_on) {
			...
			chip->usb_psy->get_property(chip->usb_psy,
				  POWER_SUPPLY_PROP_CHARGING_SOURCE, &ret);

			if (ret.intval == 2 && qpnp_chg_is_usb_chg_plugged_in(chip)) //DCP
				lge_set_chg_path_to_external();
			else if (ret.intval != 2 && qpnp_chg_is_usb_chg_plugged_in(chip)) //SDP
				lge_set_chg_path_to_internal();
#endif
...
qpnp_aicl_check_work(struct work_struct *work)
{
	...
	{
		if(wake_lock_active(&chip->aicl_wake_lock)) {
			wake_unlock(&chip->aicl_wake_lock);
		}
		pr_info("[LGE_AICL] wake_unlock , aicl_check_wake_lock = %d\n",aicl_check_wake_lock);
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->use_default_batt_values || !get_prop_batt_present(chip)) {
#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
		if(chip->batt_is_not_present) {
			pr_err("battery is absent, report zero SoC\n");
			return 0;
		}
		if(is_factory_cable()) //if battery is absent and factory cable is present, report defualt SoC
			return DEFAULT_CAPACITY;
#else
			return DEFAULT_CAPACITY;
#endif
...
#ifdef	CONFIG_MACH_MSM8926_MADAI
static int get_prop_charging_source(struct qpnp_chg_chip *chip)
{
	union power_supply_propval ret = {0,};

#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
	if(chip->batt_is_not_present)
		return BATTERY_NOT_CHARGING;//battery is not present
#endif
	chip->usb_psy->get_property(chip->usb_psy,
			  POWER_SUPPLY_PROP_CHARGING_SOURCE, &ret);

	if(!chip->usb_present || ret.intval == 0)
		return BATTERY_NOT_CHARGING;//charger is not present

	//USB_DCP_CHARGER, USB_PROPRIETARY_CHARGER, USB_FLOATED_CHARGER,
	if(ret.intval == 2 || ret.intval == 8 || ret.intval == 9)
		return BATTERY_CHARGING_AC;
	//USB_SDP_CHARGER, USB_CDP_CHARGER, USB_ACA_CHARGER,
	else
	...
	if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
		...
		if (ret.intval <= 2 && !chip->use_default_batt_values &&
			...
			if ( ret.intval != 0)
			{
				...
				qpnp_chg_iusbmax_set(chip, QPNP_CHG_I_MAX_MIN_100);
				pr_err("[LGE] current zero set usb_online: %d\n", ret.intval);
			}
#else
			if (ret.intval ==  2)
				qpnp_chg_usb_suspend_enable(chip, 1);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_SOURCE:
		val->intval = get_prop_charging_source(chip);
		break;
#endif
	default:
		return -EINVAL;
...
qpnp_eoc_work(struct work_struct *work)
{
...
#ifdef CONFIG_LGE_PM
	struct qpnp_vadc_result usb_uv, vchg_uv;
	int soc;
	u8 chg_ctrl = 0;
#endif
	...
	pr_info("chgr: 0x%x, bat_if: 0x%x, buck: 0x%x, chg_ctrl: 0x%x, usb_present: %d\n",
		chg_sts, batt_sts, buck_sts, chg_ctrl,chip->usb_present);
#else
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		rc = qpnp_vadc_read(chip->vadc_dev, USBIN, &usb_uv);
		if(rc) {
			pr_err("Unable to read usb_mv rc=%d\n", rc);
			goto check_again_later;
		}
		rc = qpnp_vadc_read(chip->vadc_dev, VCHG_SNS, &vchg_uv);
		if(rc) {
			pr_err("Unable to read vchg_mv rc=%d\n", rc);
			goto check_again_later;
		}
		pr_info("ibat_ma: %d, vbat_mv: %d, soc: %d, vbus_mV: %d, vchg_mv: %d\n",
			ibat_ma,  vbat_mv, soc, (int)usb_uv.physical/1000, (int)vchg_uv.physical/1000);
#else
...
#ifdef CONFIG_LGE_PM
stop_eoc_with_pm_relax:
	pm_relax(chip->dev);
	pr_info("wake_lock is released.\n");
#endif
...
#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
static void
qpnp_chg_battery_remove_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, battery_remove_check_work);

	if((!get_prop_batt_present(chip)) && (!is_factory_cable())) {
		chip->batt_is_not_present = true;
	}
	else {
		chip->batt_is_not_present = false;
	}
	power_supply_changed(&chip->batt_psy);
}
#endif
	...
	if (((res.change_lvl != STS_CHE_NONE) && req.is_charger) ||
		...
		if (res.change_lvl == STS_CHE_NORMAL_TO_DECCUR ||
...
#ifdef CONFIG_LGE_PM_FACTORY_TESTMODE
		else if (res.change_lvl == STS_CHE_STPCHG_TO_NORMAL && testmode_stop_eoc == 0) {
#else
...
#if defined(CONFIG_LGE_PM_CHARGING_TEMP_SCENARIO_V1_8)
#ifdef CONFIG_LGE_PM_FACTORY_TESTMODE
		else if (res.change_lvl == STS_CHE_STPCHG_TO_DECCUR && testmode_stop_eoc == 0) {
#else
			...
			wake_unlock(&chip->lcs_wake_lock);
					...
					res.dc_current != DC_CURRENT_DEF && testmode_stop_eoc == 0) {
#else
		else if (res.force_update == true && res.state == CHG_BATT_NORMAL_STATE &&
					res.dc_current != DC_CURRENT_DEF) {
#endif
...
#if defined (CONFIG_LGE_PM_CHARGING_EXTERNAL_OVP)
			gpio_request_one(chip->ext_ovp_gpio, GPIOF_OUT_INIT_LOW, "ext_ovp");
#endif
			break;
		case SMBB_DC_CHGPTH_SUBTYPE:
...
/*                                        */
	...
	wake_lock_init(&chip->aicl_wake_lock, WAKE_LOCK_SUSPEND, "qpnp_chg_aicl_check");
#endif
#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
	INIT_DELAYED_WORK(&chip->battery_remove_check_work, qpnp_chg_battery_remove_check_work);
#endif
	if (chip->dc_chgpth_base) {
...
#endif
#ifdef	CONFIG_LGE_PM_ENABLE_BATTERY_ABSENT_POWER_OFF
	cancel_delayed_work_sync(&chip->battery_remove_check_work);
#endif
