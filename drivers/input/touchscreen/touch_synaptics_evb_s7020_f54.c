...
#define F01_CMD_BASE 0x8A /*page 0x00*/ //[jungoh.seo] Register Map : 0x89
#define F01_CTRL_BASE 0x50 /*page 0x00*/
#define F01_DATA_BASE 0x13 /*page 0x00*/
#define F54_COMMAND_BASE 0x7F /*page 0x01*/ //[jungoh.seo] Register Map : 0x7B
#define F54_DATA_BASE 0x00 /*page 0x01*/
#define F54_CONTROL_BASE 0x11 //[jungoh.seo] Register Map : 0x10
#define F54_DATA_LOWINDEX F54_DATA_BASE + 1
#define F54_DATA_HIGHINDEX F54_DATA_BASE + 2
#define F54_DATA_BUFFER F54_DATA_BASE + 3
#define F54_CBC_SETTINGS F54_CONTROL_BASE + 8 //[jungoh.seo] Register Map : 0x0117, F54_CONTROL_BASE + 7 
...
int F54_evb_FullRawCap(struct i2c_client *client, int mode, char *buf)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, command);

	// Force update & Force cal
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x06);

	do {
		mdelay(1); //wait 1ms
		touch_evb_i2c_read(ts->client, F54_COMMAND_BASE, sizeof(command), &command);
	} while (command != 0x00);

	// Enabling only the analog image reporting interrupt, and turn off the rest
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x00);
	touch_evb_i2c_write_byte(ts->client, 0x14, 0x08);

	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	command = 0x00;
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE + 1, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE + 2, 0x00);

	// Set the GetReport bit to run the AutoScan
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x01);
	...
	touch_evb_i2c_read(ts->client, F54_DATA_BASE + 3, length, &ImageBuffer[0]);
	...
	//Reset
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x00);
	touch_evb_i2c_write_byte(ts->client, F01_CMD_BASE, 0x01);
	mdelay(200);
	touch_evb_i2c_read(ts->client, F01_DATA_BASE + 1, sizeof(command), &command);
...
unsigned char F54_evb_TxToTxReport(struct i2c_client *client)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x05);

	touch_evb_i2c_write_byte(ts->client, F54_DATA_LOWINDEX, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_HIGHINDEX, 0x00);
	...
	touch_evb_i2c_read(ts->client, F54_DATA_BUFFER, 2, &ImageBuffer[0]);
...
unsigned char F54_evb_TxToGndReport(struct i2c_client *client)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x10);
...
static short ImageArray2[CFG_F54_RXCOUNT][CFG_F54_RXCOUNT];
unsigned char F54_evb_RxToRxReport(struct i2c_client *client)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x07);
	touch_evb_i2c_write_byte(ts->client, F54_CBC_SETTINGS, 0x00);
	touch_evb_i2c_write_byte(ts->client, NOISE_MITIGATION, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x04);
	...
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x02);
	...
	touch_evb_i2c_read(ts->client, F54_DATA_BUFFER, length, &ImageBuffer[0]);
	...
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x11);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_LOWINDEX, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_HIGHINDEX, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x01);
...
unsigned char F54_evb_HighResistance(struct i2c_client *client, char *buf)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x04);
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x04);
	...
	touch_evb_i2c_read(ts->client, F54_DATA_BUFFER, 6, &imageBuffer[0]);
...
unsigned char F54_evb_RxOpenReport(struct i2c_client *client)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	int result = 0;
	/*short DiagonalLowerLimit = 900;*/
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x0E);
	touch_evb_i2c_write_byte(ts->client, F54_CBC_SETTINGS, 0x00);
	touch_evb_i2c_write_byte(ts->client, NOISE_MITIGATION, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x04);
	...
	length = CFG_F54_RXCOUNT * (CFG_F54_RXCOUNT-CFG_F54_TXCOUNT) * 2;
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x12);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_LOWINDEX, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_HIGHINDEX, 0x00);
	touch_evb_i2c_write_byte(ts->client, F54_COMMAND_BASE, 0x01);
...
unsigned char F54_evb_TxOpenReport(struct i2c_client *client)
{
	struct synaptics_ts_data* ts = (struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01);
	touch_evb_i2c_write_byte(ts->client, F54_DATA_BASE, 0x0F);
