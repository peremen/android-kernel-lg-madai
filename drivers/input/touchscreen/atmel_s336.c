...
static int mxt_get_cap_diff(struct mxt_data *data)
{
	...
	if(curr_crc == 0)
	{
		ret = mxt_read_config_crc(data, &curr_crc);
		if (ret) {
			TOUCH_INFO_MSG("fail to read config crc\n");
			return ret;
		}
	}
	TOUCH_INFO_MSG("cuur_crc [%d]\n",curr_crc);
	ret = mxt_write_mem(data, object->start_address + 108, 4, (u8*)&curr_crc);
	...
	//ret = mxt_read_mem(data, object->start_address,MXT_PATCH_T71_DATA_MAX, buf_t71);
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
	...
	if (t6_reset_cnt > 1 && status & MXT_T6_STATUS_RESET && global_mxt_data) {
		...
		if (mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
			...
			if (mxt_patchevent_get(PATCH_EVENT_TA)) {
				mxt_patch_event(global_mxt_data, CHARGER_KNOCKON_SLEEP + (PATCH_EVENT_PAIR_NUM * (data->g_tab_cnt - 1)));
			} else {
				mxt_patch_event(global_mxt_data, NOCHARGER_KNOCKON_SLEEP + (PATCH_EVENT_PAIR_NUM * (data->g_tab_cnt - 1)));
			}
		}
#else
		if (mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
...
int mxt_get_reference_chk(struct mxt_data *data)
{
	...
	/* butten HALL check */
#if 0
	if (data->pdata->butt_check) {
		if (err_cnt < half_err_cnt) {
			TOUCH_INFO_MSG("Check the Button Calibration\n");
			for (i=0; i < data->pdata->t15_num_keys; i++) {
				TOUCH_INFO_MSG("reference : %d ABS_Diff : %d x[%d] y[%d]",
					buf[data->pdata->t15_key_array_x[i] + data->pdata->t15_key_array_y[i]*data->info->matrix_ysize],
					jitter_sub(key_button_rawdata[i], buf[data->pdata->t15_key_array_x[i] + data->pdata->t15_key_array_y[i]*data->info->matrix_ysize]),
					data->pdata->t15_key_array_x[i], data->pdata->t15_key_array_y[i]);
				/* button channel check */
				if (jitter_sub(key_button_rawdata[i], buf[data->pdata->t15_key_array_x[i] + data->pdata->t15_key_array_y[i]*data->info->matrix_ysize]) > 2000) {
				...
				if (jitter_sub(key_button_rawdata[i], buf[data->pdata->t15_key_array_x[i] + data->pdata->t15_key_array_y[i]*data->info->matrix_ysize]) > BUTT_CAL_OFFSET) {
					TOUCH_INFO_MSG("%s Button HALL Occur!", get_touch_button_string(data->pdata->t15_keymap[i]));
					mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
				}
			}
		}
...
retry:
	...
	if (data->g_tab_cnt != tap_num && data->mxt_multi_tap_enable) {
	    TOUCH_INFO_MSG("Tab count dismatch t37[%d] tap number is %d\n", data->g_tab_cnt, tap_num);
	    goto error;
	} else {
...
void trigger_usb_state_from_otg(int usb_type)
{
	...
	if (global_mxt_data && global_mxt_data->patch.event_cnt) {
		...
		if (usb_type == 0) {
...
#if defined(CONFIG_TOUCHSCREEN_LGE_LPWG)
					mxt_patch_event(global_mxt_data, CHARGER_KNOCKON_WAKEUP + (PATCH_EVENT_PAIR_NUM * (global_mxt_data->g_tab_cnt - 1)));
#else
			...
			if (mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
#if defined(CONFIG_TOUCHSCREEN_LGE_LPWG)
				mxt_patch_event(global_mxt_data, NOCHARGER_KNOCKON_WAKEUP + (PATCH_EVENT_PAIR_NUM * (global_mxt_data->g_tab_cnt - 1)));
#else
		...
		error = __mxt_read_reg(global_mxt_data->client, global_mxt_data->T71_address + 51, 23, &global_mxt_data->ref_limit);
		if (!error) {
			...
			TOUCH_INFO_MSG("Err Range y: %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
...
static int mxt_init_t93_tab_count(struct mxt_data *data)
{
	...
	if (object) {
		__mxt_read_reg(data->client, object->start_address + 17, 1, &(data->g_tab_cnt));
		TOUCH_INFO_MSG("%s data->g_tab_cnt : %d\n", __func__, data->g_tab_cnt);
		return 0;
	} else {
		data->g_tab_cnt = 0;
		return 1;
...
#if defined(CONFIG_TOUCHSCREEN_LGE_LPWG)
		if (mxt_patchevent_get(PATCH_EVENT_TA)) {
			if (data->lpwg_mode || mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
				mxt_patch_event(global_mxt_data, CHARGER_KNOCKON_WAKEUP + (PATCH_EVENT_PAIR_NUM * (data->g_tab_cnt - 1)));
			}
		} else {
			if (data->lpwg_mode || mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
				mxt_patch_event(global_mxt_data, NOCHARGER_KNOCKON_WAKEUP + (PATCH_EVENT_PAIR_NUM * (data->g_tab_cnt - 1)));
			}
		}
#else
		if (mxt_patchevent_get(PATCH_EVENT_TA)) {
...
static int mxt_write_config(struct mxt_fw_info *fw_info)
{
	...
	TOUCH_INFO_MSG("t71_cfg_crc[%06X] ,fw_info->cfg_crc[%06X]\n",t71_cfg_crc,fw_info->cfg_crc);
	...
	if(buf_crc_t71[0] == 1)
	{
		...
		ret = mxt_write_mem(data, object->start_address + 60, 14, (u8*)&data->t71_diff_val);
		if (ret) {
