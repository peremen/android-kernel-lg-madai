...
{
	return touch_evb_i2c_write(client, uRmiAddress, length, data);
}

static int SynaReadRegister(struct i2c_client *client,
		u8 uRmiAddress, u8 * data, unsigned int length)
{
	return touch_evb_i2c_read(client, uRmiAddress, length, data);
}
...
{
	if (touch_evb_debug_mask & DEBUG_FW_UPGRADE)
...
static int RMI4ReadPageDescriptionTable(struct synaptics_fw_data *fw, struct synaptics_ts_data *ts)
{
	...
	fw->m_uF01RMI_DataBase = fw->m_PdtF01Common.m_DataBase;//0x0013
	fw->m_uF01RMI_IntStatus = fw->m_PdtF01Common.m_DataBase+1;//0x0014
	fw->m_uF01RMI_CommandBase = fw->m_PdtF01Common.m_CommandBase;//0x0089
	fw->m_uF01RMI_QueryBase = fw->m_PdtF01Common.m_QueryBase;//0x0094

	fw->m_uF34Reflash_DataReg = fw->m_PdtF34Flash.m_DataBase;//0x0000
	fw->m_uF34Reflash_BlockNum = fw->m_PdtF34Flash.m_DataBase;//0x0000
	fw->m_uF34Reflash_BlockData = fw->m_PdtF34Flash.m_DataBase+2;//0x0002
	fw->m_uF34ReflashQuery_BootID = fw->m_PdtF34Flash.m_QueryBase;//0x008B

	fw->m_uF34ReflashQuery_FlashPropertyQuery = fw->m_PdtF34Flash.m_QueryBase+2;//0x008D
	fw->m_uF34ReflashQuery_FirmwareBlockSize = fw->m_PdtF34Flash.m_QueryBase+3;//0x008E
	fw->m_uF34ReflashQuery_FirmwareBlockCount = fw->m_PdtF34Flash.m_QueryBase+5;//0x0090
	fw->m_uF34ReflashQuery_ConfigBlockSize = fw->m_PdtF34Flash.m_QueryBase+3;//0x008E
	fw->m_uF34ReflashQuery_ConfigBlockCount = fw->m_PdtF34Flash.m_QueryBase+7;//0x0092
 ...
 */
int FirmwareUpgradeEvb(struct synaptics_ts_data *ts, const char* fw_path)
{
