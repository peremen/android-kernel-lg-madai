...
void write_firmware_version_log(struct synaptics_ts_data *ts)
{
	...
	if(!(strncmp(ts->fw_info.fw_product_id, S3320_PRODUCT_ID, strlen(S3320_PRODUCT_ID)))|| !(strncmp(ts->fw_info.fw_product_id, "PLG430", 6))) {
...
static ssize_t show_firmware(struct i2c_client *client, char *buf)
{
	...
	if (!(strncmp(ts->fw_info.fw_image_product_id, S3320_PRODUCT_ID, strlen(S3320_PRODUCT_ID)))|| !(strncmp(ts->fw_info.fw_product_id, "PLG430", 6))) {
		ret += sprintf(buf+ret, "Touch IC : s3320(family_id = %d, fw_rev = %d)\n",
			ts->fw_info.family, ts->fw_info.fw_revision);		
...
static int get_ic_info(struct synaptics_ts_data *ts)
{
	...
	if (!(strncmp(ts->fw_info.fw_product_id, "PLG313", 6))
		...
		|| !(strncmp(ts->fw_info.fw_product_id, S3320_PRODUCT_ID, strlen(S3320_PRODUCT_ID)))|| !(strncmp(ts->fw_info.fw_product_id, "PLG430", 6))) {
		if (ts->fw_info.family == 0 && ts->fw_info.fw_revision == 0) {
			TOUCH_DEBUG(DEBUG_BASE_INFO, "!!fw_image_s3320 = [%s]\n", ts->pdata->inbuilt_fw_name);
				...
				const struct state_info *state, struct attribute ***attribute_list)
{
	...
	*attribute_list = lge_specific_touch_attribute_list;
	...
	//okb
	printk("[Touch_okb] %s: ts->lpwg_ctrl.mode = %d \n", __func__,ts->lpwg_ctrl.lpwg_mode );
...
enum error_type synaptics_ts_init(struct i2c_client *client)
{
	...
	} else if (!(strncmp(ts->fw_info.fw_product_id, S3320_PRODUCT_ID, strlen(S3320_PRODUCT_ID)))|| !(strncmp(ts->fw_info.fw_product_id, "PLG430", 6))) {
		...
		for (i = 0; i < MAX_NUM_OF_FINGERS; i++) {
			if (ts->ts_data.finger.finger_reg[i][0] == F12_FINGER_STATUS
				...
				curr_data->abs_data[finger_index].raw_x =
					ABS_M(TS_SNTS_GET_X_POSITION(ts->ts_data.finger.finger_reg[i][REG_X_MSB],
							       ts->ts_data.finger.finger_reg[i][REG_X_LSB])-720);
				curr_data->abs_data[finger_index].y =
				curr_data->abs_data[finger_index].raw_y =
					ABS_M(TS_SNTS_GET_Y_POSITION(ts->ts_data.finger.finger_reg[i][REG_Y_MSB],
							       ts->ts_data.finger.finger_reg[i][REG_Y_LSB])-1280);
			...
			*/
			...
			if (ts->pdata->reset_pin > 0) {
				gpio_tlmm_config(GPIO_CFG(ts->pdata->reset_pin, 0, GPIO_CFG_OUTPUT,
					GPIO_CFG_NO_PULL, GPIO_CFG_6MA), GPIO_CFG_ENABLE);
...
//compare :
	...
	if((!strncmp("PLG430", ts->fw_info.fw_image_product_id, sizeof(ts->fw_info.fw_image_product_id)))
		&&(!strncmp("S3320B", ts->fw_info.fw_product_id, sizeof(ts->fw_info.fw_product_id)))) {
		need_upgrade = 1;
	} else if((!strncmp("PLG430", ts->fw_info.fw_image_product_id, sizeof(ts->fw_info.fw_image_product_id)))
		&&(!strncmp("s3320", ts->fw_info.fw_product_id, sizeof(ts->fw_info.fw_product_id)))) {
		need_upgrade = 1;
	}	
	TOUCH_DEBUG(DEBUG_BASE_INFO | DEBUG_FW_UPGRADE, "FW:[%s] fw_prodcut[%s]\n",ts->fw_info.fw_image_product_id,ts->fw_info.fw_product_id);
...
enum error_type synaptics_ts_suspend(struct i2c_client *client)
{
	...
	DO_SAFE(touch_i2c_write_byte(client, DEVICE_CONTROL_REG,
			DEVICE_CONTROL_SLEEP | DEVICE_CONTROL_CONFIGURED), error);
	//because s3621 doesn't support knock-on
	if(!(strncmp(ts->fw_info.fw_product_id, "PLG430",6)) || !(strncmp(ts->fw_info.fw_product_id, S3320_PRODUCT_ID, strlen(S3320_PRODUCT_ID))))
	{
	...
	if (!atomic_read(&ts->lpwg_ctrl.is_suspend)) {
		TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : lpwg_ctrl.is_suspend is '0'",__func__);
