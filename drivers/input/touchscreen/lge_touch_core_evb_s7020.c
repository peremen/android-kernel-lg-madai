...
#include <mach/gpio.h>
#endif
#include <linux/input/lge_touch_core_evb_s7020.h>
...
#include "touch_synaptics_evb_s7020_f54.h"
#endif
#endif
...
static struct touch_device_driver*	touch_device_func;
 ...
 */
u32 touch_evb_debug_mask = DEBUG_BASE_INFO;
//u32 touch_evb_debug_mask = DEBUG_TRACE | DEBUG_BASE_INFO;
//u32 touch_evb_debug_mask = DEBUG_TRACE | DEBUG_GET_DATA | DEBUG_BUTTON | DEBUG_BASE_INFO | DEBUG_ABS | DEBUG_POWER;

module_param_named(debug_mask, touch_evb_debug_mask, int, S_IRUGO|S_IWUSR|S_IWGRP);
 ...
 */
u32 touch_evb_time_debug_mask = DEBUG_TIME_PROFILE_NONE;
module_param_named(time_debug_mask, touch_evb_time_debug_mask, int, S_IRUGO|S_IWUSR|S_IWGRP);
...
static struct t_data	 ts_prev_finger_press_data;

static int trigger_baseline = 0;
//static int ts_pen_ctrl = 0;
...
struct lge_touch_data *touch_evb_test_dev = NULL;
EXPORT_SYMBOL(touch_evb_test_dev);
...
#if defined(CONFIG_TS_INFO_CLASS) && defined(CONFIG_MACH_MSM8960_L1m)
static void synaptics_ts_classdev_register(void);
#endif
...
bool block_interrupt;
	...
	#if defined(CONFIG_MACH_MSM8960_FX1S) || defined(CONFIG_MACH_MSM8960_FX1SU) || defined(CONFIG_MACH_MSM8926_MADAI_S) 
static bool block_interrupt;
	#endif
#endif

void Send_Touch_evb( unsigned int x, unsigned int y)
{
	if (touch_evb_test_dev)
	{
		/* press */
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_POSITION_X, ABS_M(x-1536));
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_POSITION_Y, ABS_M(y-2560));
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_PRESSURE, 1);
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_WIDTH_MAJOR, 1);
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_WIDTH_MINOR, 1);
		input_mt_sync(touch_evb_test_dev->input_dev);
		input_sync(touch_evb_test_dev->input_dev);

		/* release */
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_POSITION_X, ABS_M(x-1536));
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_POSITION_Y, ABS_M(y-2560));
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_PRESSURE, 0);
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_WIDTH_MAJOR, 0);
		input_report_abs(touch_evb_test_dev->input_dev, ABS_MT_WIDTH_MINOR, 0);
		input_mt_sync(touch_evb_test_dev->input_dev);
		input_sync(touch_evb_test_dev->input_dev);
	}
	else
...
static ssize_t store_send_touch_down(struct lge_touch_data *ts, const char *buf, size_t count)
{
       ...
       if (ts){
              input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ABS_M(x-1536));
              input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ABS_M(y-2560));
...
int get_touch_evb_ts_fw_version(char *fw_ver)
{
	if (touch_evb_test_dev)
	{
		sprintf(fw_ver, "%d.0%d", touch_evb_test_dev->fw_info.manufacturer_id,
				touch_evb_test_dev->fw_info.fw_rev);
		return 1;
	}
	else
	{
...
EXPORT_SYMBOL(get_touch_evb_ts_fw_version);


/* set_touch_evb_handle / get_touch_evb_handle
 *
 * Developer can save their object using 'set_touch_evb_handle'.
 * Also, they can restore that using 'get_touch_evb_handle'.
 */
void set_touch_evb_handle(struct i2c_client *client, void* h_touch)
{
	struct lge_touch_data *ts = i2c_get_clientdata(client);
	printk("[touch] i2c_get_clientdata END\n");
	ts->h_touch = h_touch;
}
...
/* touch_evb_i2c_read / touch_evb_i2c_write
 *
...
{
	if (touch_evb_time_debug_mask & DEBUG_TIME_PROFILE_ALL) {
		...
		if (touch_evb_time_debug_mask & DEBUG_TIME_INT_INTERVAL) {
			if (t_debug[TIME_INT_INTERVAL].tv_sec == 0
					&& t_debug[TIME_INT_INTERVAL].tv_usec == 0) {
				t_debug[TIME_INT_INTERVAL].tv_sec = t_debug[TIME_ISR_START].tv_sec;
				t_debug[TIME_INT_INTERVAL].tv_usec = t_debug[TIME_ISR_START].tv_usec;
		...
		if (touch_evb_time_debug_mask & DEBUG_TIME_INT_IRQ_DELAY) {
			TOUCH_INFO_MSG("IRQ -> Thread IRQ : %6luus\n",
				get_time_interval(t_debug[TIME_THREAD_ISR_START].tv_usec, t_debug[TIME_ISR_START].tv_usec));
		}

		if (touch_evb_time_debug_mask & DEBUG_TIME_INT_THREAD_IRQ_DELAY) {
			TOUCH_INFO_MSG("Thread IRQ -> work: %6luus\n",
				get_time_interval(t_debug[TIME_WORKQUEUE_START].tv_usec, t_debug[TIME_THREAD_ISR_START].tv_usec));
		}

		if (touch_evb_time_debug_mask & DEBUG_TIME_DATA_HANDLE) {
...
static void release_all_ts_event(struct lge_touch_data *ts)
{
	if (ts->pdata->role->key_type == TOUCH_HARD_KEY){
		if (ts->ts_data.prev_total_num) {
			...
			if (likely(touch_evb_debug_mask & (DEBUG_ABS | DEBUG_BASE_INFO)))
		...
		if(ts->ts_data.prev_button.state == BUTTON_PRESSED) {
			...
			if (likely(touch_evb_debug_mask & (DEBUG_BUTTON | DEBUG_BASE_INFO)))
...
static int touch_power_cntl(struct lge_touch_data *ts, int onoff)
{
	...
	if (unlikely(touch_evb_debug_mask & DEBUG_POWER))
		if (ret >= 0)
...
{
	if (touch_evb_test_dev->pdata->role->ghost_detection_enable) {
		if(trigger_baseline==1)
		{
			trigger_baseline = 2;
			atomic_inc(&touch_evb_test_dev->next_work);
			queue_work(touch_wq, &touch_evb_test_dev->work);
		}
	}
...
#if 0
static void trigger_baseline_state_machine(int plug_in)
{
	if(touch_evb_test_dev->gf_ctrl.probe != 1)
		return;

#if defined(CONFIG_MACH_MSM8926_MADAI_M)
	if (touch_evb_test_dev && touch_evb_test_dev->pdata->role->ghost_detection_enable) {
#else
	if (touch_evb_test_dev->pdata->role->ghost_detection_enable) {
#endif
		if(plug_in == 0 || plug_in == 1)    // TA/USB connected or disconnected
		{
			...
			if(touch_evb_test_dev && atomic_read(&touch_evb_test_dev->device_init) == 1 && trigger_baseline==0 ){
				trigger_baseline = 1;
    ...
    else if (plug_in ==1)   // MHL connected
    {
        u8 buff=0;
        ts_pen_ctrl = 1;
        touch_evb_i2c_read(touch_evb_test_dev->client, 0x83, 1, &buff);
        buff = buff&0xFB;
        touch_evb_i2c_write_byte(touch_evb_test_dev->client, 0x83, buff);
...
#if !defined(CONFIG_MACH_MSM8960_L1v) && !defined(CONFIG_MACH_MSM8926_MADAI_M)
...
#endif
	if (likely(touch_evb_debug_mask & (DEBUG_BASE_INFO | DEBUG_GHOST))){
...
static int ghost_finger_solution(struct lge_touch_data *ts)
{
	...
	if(ts->gf_ctrl.stage & GHOST_STAGE_1){
		if(ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0 && ts->ts_data.palm_touch == 0){
			...
			if (unlikely(touch_evb_debug_mask & DEBUG_GHOST))
			...
			if(ts->gf_ctrl.ghost_check_count >= MAX_GHOST_CHECK_COUNT){
				...
				if(!ts->gf_ctrl.stage){
					if (unlikely(touch_evb_debug_mask & DEBUG_GHOST
							|| touch_evb_debug_mask & DEBUG_BASE_INFO))
...
static int trigger_baseline = 0;
static int ts_chg_ctrl = 0;
static void trigger_baseline_state_machine(int plug_in)
{
	if(touch_evb_test_dev && atomic_read(&touch_evb_test_dev->device_init) == 1){
		trigger_baseline = 1;
		ts_chg_ctrl = plug_in;
	}
}
...
static int accuracy_filter_func(struct lge_touch_data *ts)
{
	...
	if(unlikely(touch_evb_debug_mask & DEBUG_ACCURACY)){
...
//				   TOUCH_INFO_MSG("JitterFilter: release-all\n");
	...
	for(i=0; i<ts->ts_data.total_num; i++){
		...
		if(ts->jitter_filter.id_mask & (1 << id)){
			...
			if(unlikely(touch_evb_debug_mask & DEBUG_JITTER)){
...
static void touch_init_func(struct work_struct *work_init)
{
	...
	if (unlikely(touch_evb_debug_mask & DEBUG_TRACE))
		TOUCH_DEBUG_MSG("\n");
...
static void touch_work_func_a(struct work_struct *work)
{
	...
	if (ts->ts_data.state != TOUCH_ABS_LOCK) {
		if (!ts->ts_data.total_num) {
			...
			if (likely(touch_evb_debug_mask & (DEBUG_BASE_INFO | DEBUG_ABS))) {
				if (ts->ts_data.prev_total_num) {
					for(cnt=0; cnt < MAX_FINGER; cnt++){
					...
					if(cnt < 10) {
						TOUCH_INFO_MSG("touch_release : <%d> x[%4d] y[%4d]\n",
								cnt, ABS_M(ts->ts_data.prev_data[cnt].x_position-1536),
								ABS_M(ts->ts_data.prev_data[cnt].y_position-2560));
					}
				}
			}
...
//			if (ts->gf_ctrl.stage == GHOST_STAGE_CLEAR || ts->gf_ctrl.stage == GHOST_STAGE_3)
					...
					if (ts->ts_data.prev_total_num <= ts->ts_data.total_num) {
						...
						if(cnt < 10) {
							TOUCH_INFO_MSG("%d finger pressed : <%d> x[%4d] y[%4d] p[%3d]\n",
									...
									ABS_M(ts->ts_data.curr_data[cnt].x_position-1536),
									ABS_M(ts->ts_data.curr_data[cnt].y_position-2560),
			...
			while(ts->ts_data.total_num--) {
				if ((cnt < MAX_FINGER) && (ts->ts_data.curr_data[cnt].status == FINGER_PRESSED)) {
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
							ABS_M(ts->ts_data.curr_data[ts->ts_data.total_num].x_position-1536));
					input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
							ABS_M(ts->ts_data.curr_data[ts->ts_data.total_num].y_position-2560));
					...
					if (unlikely(touch_evb_debug_mask & DEBUG_ABS))
	...
	if (ts->ts_data.state != TOUCH_BUTTON_LOCK) {
		...
		if (ts->work_sync_err_cnt > 0
			...
			report_enable = 0;
#if 0
			if (unlikely(touch_evb_debug_mask & DEBUG_BUTTON))
...
err_out_critical:
	TOUCH_INFO_MSG("err_out_critical\n");
	ts->work_sync_err_cnt = 0;
	safety_reset(ts);
	touch_ic_init(ts);
	//mutex_unlock(&ts->irq_work_mutex);
...
//	if (unlikely(touch_evb_debug_mask & (DEBUG_ABS |DEBUG_BUTTON)))
...
abs_report:
		...
		if (!i) {
			...
			if (likely(touch_evb_debug_mask & DEBUG_ABS)) {
	...
	if (likely(touch_evb_debug_mask & DEBUG_BASE_INFO)){
...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
...
#else
	if (likely(touch_evb_debug_mask & (DEBUG_FW_UPGRADE | DEBUG_BASE_INFO)))
...
#elif defined(CONFIG_MACH_MSM8926_MADAI_M)
	/*                                                                    
  ...
  */
	...
	if(board_type == HW_REV_A) {
	    ...
	    goto out;
	  #elif defined(CONFIG_MACH_MSM8926_MADAI_S)
...
/*	else if (!strncmp(ts->fw_info.product_id , "PLG126:S7020", 12))
	...
	if (touch_evb_time_debug_mask & DEBUG_TIME_FW_UPGRADE
			|| touch_evb_time_debug_mask & DEBUG_TIME_PROFILE_ALL) {
 ...
 */
static void get_section(struct section_info* sc, struct touch_platform_data *pdata)
{
	int i;
...
static ssize_t show_platform_data(struct lge_touch_data *ts, char *buf)
{
	...
	ret += sprintf(buf+ret, "\tbutton_name           = %d\n", pdata->caps->button_name[0]);
...
static ssize_t store_keyguard_info(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	if (touch_evb_debug_mask & DEBUG_GHOST){
...
{
	int value;
	TOUCH_INFO_MSG("store_incoming_call Enter\n");
	sscanf(buf, "%d", &value);
...
{
	int ret = 0;
	ret += F54_evb_FullRawCap(ts->client, 3, buf);
	mdelay(100);
	ret += F54_evb_HighResistance(ts->client, buf + ret);
	return ret;
}
...
static ssize_t store_F54(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	switch(value){
		case 0:
			...
			break;
		case 1:
			F54_evb_FullRawCap(ts->client, 3, temp);
			break;
		case 2:
			F54_evb_FullRawCap(ts->client, 4, temp);
			break;
		case 3:
			F54_evb_TxToTxReport(ts->client);
			break;
		case 4:
			F54_evb_TxToGndReport(ts->client);
			break;
		case 5:
			F54_evb_RxToRxReport(ts->client);
			break;
		case 6:
			F54_evb_HighResistance(ts->client, temp);
			break;
		case 7:
			F54_evb_RxOpenReport(ts->client);
			break;
		case 8:
			F54_evb_TxOpenReport(ts->client);
			break;
		default:
...
static void synaptics_ts_classdev_register(void)
{
        ...
        classdev_version[0] = touch_evb_test_dev->fw_info.fw_config_id[0];
        classdev_version[1] = touch_evb_test_dev->fw_info.fw_config_id[1];
        classdev_version[2] = touch_evb_test_dev->fw_info.fw_config_id[2];
        classdev_version[3] = touch_evb_test_dev->fw_info.fw_config_id[3];
	classdev_version[4] = 0;

        cdev.version = classdev_version;
        ts_info_classdev_register(&touch_evb_test_dev->client->dev, &cdev);
}
#endif

int synaptics_evb_power_on(struct i2c_client *client, int on)
{
	int rc = 0;
	...
	static struct regulator *vreg_l6;
		...
		if (!vreg_l28) {
			vreg_l28 = regulator_get(&client->dev, "vdd_ana");
			if (IS_ERR(vreg_l28)) {
		...
		if (!vreg_l6) {
			vreg_l6 = regulator_get(&client->dev, "vcc_i2c");
			if (IS_ERR(vreg_l6)) {
			...
			rc = regulator_set_voltage(vreg_l28, 2800000, 2800000);
...
*/
	...
	/* pwr */
		...
		pwr_info->power = synaptics_evb_power_on;
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[touch] mutex_init irq_work_mutex \n");
#endif
...
int touch_evb_driver_register(struct touch_device_driver* driver,
	struct of_device_id *match_table)
{
	...
	if (touch_device_func != NULL) {
		...
		ret = -EMLINK;
		goto err_touch_evb_driver_register;
	}
