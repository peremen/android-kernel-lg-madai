...
#include <linux/input/touch_synaptics_evb_s7020.h>
...
#define BUTTON_CONTROL_REG			(ts->button_dsc.control_base)//0x0201
...
u8 pressure_zero_evb = 0;

int synaptics_ts_evb_get_data(struct i2c_client *client, struct t_data* data, struct b_data* button, u8* total_num, u8* palm_touch)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
	...
	*total_num = 0;
	...
	if (unlikely(touch_evb_i2c_read(client, DEVICE_STATUS_REG,
	...
	if (unlikely(touch_evb_i2c_read(client, INTERRUPT_STATUS_REG,
			sizeof(ts->ts_data.interrupt_status_reg),
	...
	if (unlikely(touch_evb_debug_mask & DEBUG_GET_DATA))
	...
	if (likely(ts->ts_data.interrupt_status_reg & ts->interrupt_mask.abs)) {
		if (unlikely(touch_evb_i2c_read(client, FINGER_STATE_REG,
		...
		while(touch_finger_bit_mask) {
			...
			if (unlikely(touch_evb_i2c_read(ts->client,
			...
			if(ts->pdata->role->ghost_detection_enable) {
				if(data[index].pressure == 0)	pressure_zero_evb = 1;
			}
	...
	if (unlikely(ts->button_dsc.id != 0)) {
		if (likely(ts->ts_data.interrupt_status_reg & ts->interrupt_mask.button)) {
			if (unlikely(touch_evb_i2c_write_byte(client, PAGE_SELECT_REG, 0x02) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
			}

			if (unlikely(touch_evb_i2c_read(client, BUTTON_DATA_REG,
...
*/
			...
			if (unlikely(touch_evb_i2c_write_byte(client, PAGE_SELECT_REG, 0x00) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
			}
		}
	}
	if(unlikely(touch_evb_i2c_read(client, TWO_D_EXTEND_STATUS, 1, &buf))){
		TOUCH_ERR_MSG();
		goto err_synaptics_getdata;
	}
	*palm_touch = buf & 0x02;
...
static int read_page_description_table(struct i2c_client* client)
{
	...
	if (touch_evb_debug_mask & DEBUG_TRACE)
		TOUCH_DEBUG_MSG("\n");
	...
	for(u_address = DESCRIPTION_TABLE_START; u_address > 10; u_address -= sizeof(struct ts_function_descriptor)) {
		if (unlikely(touch_evb_i2c_read(client, u_address, sizeof(buffer), (unsigned char *)&buffer) < 0)) {
	...
	if (unlikely(touch_evb_i2c_write_byte(client, PAGE_SELECT_REG, 0x01) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
			}
		for(u_address = DESCRIPTION_TABLE_START; u_address > 10; u_address -= sizeof(struct ts_function_descriptor)) {
	...
	if (touch_evb_debug_mask & DEBUG_BASE_INFO)
...
int get_ic_info_evb(struct synaptics_ts_data* ts, struct touch_fw_info* fw_info)
{
	u8 device_status = 0;
	u8 flash_control = 0;
	u8 analog_control = 0;
	...
	u8 i = 0;
#elif defined (CONFIG_MACH_MSM8926_MADAI_M)
    #if defined(CONFIG_MACH_MSM8960_FX1S) || defined(CONFIG_MACH_MSM8960_FX1SU) || defined(CONFIG_MACH_MSM8926_MADAI_S) || defined(CONFIG_MACH_MSM8960_FX1T) || defined(CONFIG_MACH_MSM8960_FX1A)
	...
	fw_info->fw_img_config_id[4] = 0;
		...
		if(board_type >= HW_REV_B){
			...
			board_type = lge_get_board_revno();
			strncpy(fw_info->fw_image_product_id, &SynaFirmware_evb_s7020[16], 10);
			fw_info->fw_start = (unsigned char *)&SynaFirmware_evb_s7020[0];
			fw_info->fw_size = sizeof(SynaFirmware_evb_s7020);
	#else
		strncpy(fw_info->fw_image_product_id, &SynaFirmware[16], 10);
		fw_info->fw_start = (unsigned char *)&SynaFirmware[0];
		fw_info->fw_size = sizeof(SynaFirmware);
	...
	if(fw_info->fw_size > 0xb104)
	{
		...
		{	
			...
			strncpy(fw_info->fw_img_config_id, &SynaFirmware_evb_s7020[0xb100], 4);
		}	
	#else
	...
	if (unlikely(touch_evb_i2c_read(ts->client, FLASH_CONTROL_REG, sizeof(flash_control), &flash_control) < 0)) {
		TOUCH_ERR_MSG("FLASH_CONTROL_REG read fail\n");
		return -EIO;
	}

	if (unlikely(touch_evb_i2c_read(ts->client, DEVICE_STATUS_REG, sizeof(device_status), &device_status) < 0)) {
		TOUCH_ERR_MSG("DEVICE_STATUS_REG read fail\n");
		return -EIO;
	}
	if (unlikely(touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x01) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
			}

	if (unlikely(touch_evb_i2c_read(ts->client, ANALOG_CONTROL_REG, sizeof(analog_control), &analog_control) < 0)) {
		TOUCH_ERR_MSG("ANALOG_CONTROL_REG read fail\n");
		return -EIO;
	}
	if (unlikely(touch_evb_i2c_write_byte(ts->client, PAGE_SELECT_REG, 0x00) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
			}
...
int synaptics_ts_evb_init(struct i2c_client* client, struct touch_fw_info* fw_info)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
...
#if defined(CONFIG_MACH_MSM8960_D1L_KR) || defined(CONFIG_MACH_MSM8960_L1) || defined (CONFIG_MACH_MSM8926_MADAI_M)
	if(ts == NULL){
	...
	if (!ts->is_probed)
		if (unlikely(get_ic_info_evb(ts, fw_info) < 0))
			return -EIO;
	...
	if (unlikely(touch_evb_i2c_write_byte(client, DEVICE_CONTROL_REG,
	...
	if (unlikely(touch_evb_i2c_read(client, INTERRUPT_ENABLE_REG,
			1, &buf) < 0)) {
		TOUCH_ERR_MSG("INTERRUPT_ENABLE_REG read fail\n");
		return -EIO;
	}
	if (unlikely(touch_evb_i2c_write_byte(client, INTERRUPT_ENABLE_REG,
	...
	if(ts->pdata->role->report_mode == CONTINUOUS_REPORT_MODE) {
		printk("[TOUCH] %s : CONTINUOUS_REPORT_MODE(%d)\n", __FUNCTION__,ts->pdata->role->report_mode);
		if (unlikely(touch_evb_i2c_write_byte(client, TWO_D_REPORTING_MODE,
				REPORT_MODE_CONTINUOUS) < 0)) {
			TOUCH_ERR_MSG("TWO_D_REPORTING_MODE write fail\n");
			return -EIO;
		}
	} else {	/* REDUCED_REPORT_MODE */
		printk("[TOUCH] %s : REDUCED_REPORT_MODE(%d)\n", __FUNCTION__, ts->pdata->role->report_mode);
		if (unlikely(touch_evb_i2c_write_byte(client, TWO_D_REPORTING_MODE,
		...
		if (unlikely(touch_evb_i2c_write_byte(client, DELTA_X_THRESH_REG,
				ts->pdata->role->delta_pos_threshold) < 0)) {
			TOUCH_ERR_MSG("DELTA_X_THRESH_REG write fail\n");
			return -EIO;
		}
		if (unlikely(touch_evb_i2c_write_byte(client, DELTA_Y_THRESH_REG,
				ts->pdata->role->delta_pos_threshold) < 0)) {
	...
	if (unlikely(touch_evb_i2c_write_byte(ts->client, ANALOG_CONTROL_REG + 170, 0x14) < 0)) {
	...
	if(1){//lge_get_board_revno() < HW_REV_1_2){
		if (unlikely(touch_evb_i2c_write_byte(client, BUTTON_PRESS_THRESH_REG_0, BUTTON_PRESS_THRESHOLD) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}

		if (unlikely(touch_evb_i2c_read(client, BUTTON_PRESS_THRESH_REG_0, 1, &buf) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		...
		if (unlikely(touch_evb_i2c_write_byte(client, BUTTON_PRESS_THRESH_REG_1, BUTTON_PRESS_THRESHOLD) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}

		if (unlikely(touch_evb_i2c_read(client, BUTTON_PRESS_THRESH_REG_1, 1, &buf) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		...
		if (unlikely(touch_evb_i2c_write_byte(client, BUTTON_RELEASE_THRESH_REG, BUTTON_PRESS_THRESHOLD) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}

		if (unlikely(touch_evb_i2c_read(client, BUTTON_CONTROL_REG, 1, &buf) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
...
/* Blocked control Button control register
	if (unlikely(touch_evb_i2c_write_byte(client, BUTTON_CONTROL_REG+13, 0x89) < 0)) {
		TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
		return -EIO;
	}

	if (unlikely(touch_evb_i2c_read(client, BUTTON_CONTROL_REG+13, 1, &buf) < 0)) {
		TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
		return -EIO;
	}
...
*/
	...
	if (unlikely(touch_evb_i2c_read(client, INTERRUPT_STATUS_REG, 1, &buf) < 0)) {
		TOUCH_ERR_MSG("INTERRUPT_STATUS_REG read fail\n");
		return -EIO;	// it is critical problem because interrupt will not occur.
	}

	if (unlikely(touch_evb_i2c_read(client, FINGER_STATE_REG, sizeof(ts->ts_data.finger.finger_status_reg),
			ts->ts_data.finger.finger_status_reg) < 0)) {
		TOUCH_ERR_MSG("FINGER_STATE_REG read fail\n");
...
int synaptics_ts_evb_power(struct i2c_client* client, int power_ctrl)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
...
//int synaptics_ts_evb_probe(struct i2c_client* client)
int synaptics_ts_evb_probe(struct lge_touch_data* lge_touch_ts)
	
{
	...
	set_touch_evb_handle(client, ts);
...
void synaptics_ts_evb_remove(struct i2c_client* client)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
...
int synaptics_ts_evb_fw_upgrade(struct i2c_client* client, const char* fw_path)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
	int ret = 0;
	...
	ret = FirmwareUpgradeEvb(ts, fw_path);

	/* update IC info */
	get_ic_info_evb(ts, ts->fw_info);

	return ret;
}

int synaptics_ts_evb_ic_ctrl(struct i2c_client *client, u8 code, u16 value)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_evb_handle(client);
	u8 buf = 0;
	...
	case IC_CTRL_BASELINE:
		...
		case BASELINE_OPEN:
			...
			if (unlikely(touch_evb_i2c_write_byte(client, ANALOG_CONTROL_REG, FORCE_FAST_RELAXATION) < 0)) {
			...
			if (unlikely(touch_evb_i2c_write_byte(client, ANALOG_COMMAND_REG, FORCE_UPDATE) < 0)) {
				TOUCH_ERR_MSG("FFR_CONTROL_REG write fail\n");
				return -EIO;
			}
		...
		case BASELINE_FIX:
			...
			if (unlikely(touch_evb_i2c_write_byte(client, ANALOG_CONTROL_REG, 0x00) < 0)) {
				TOUCH_ERR_MSG("FFR_CONTROL_REG write fail\n");
				return -EIO;
			}
		...
		case BASELINE_REBASE:
			...
			if (likely(ts->finger_dsc.id != 0)) {
				if (unlikely(touch_evb_i2c_write_byte(client, FINGER_COMMAND_REG, 0x1) < 0)) {
			...
			if (unlikely(ts->button_dsc.id != 0)) {
				if (unlikely(touch_evb_i2c_write_byte(client, BUTTON_COMMAND_REG, 0x1) < 0)) {
					TOUCH_ERR_MSG("finger baseline reset command write fail\n");
					return -EIO;
				}
			}
		...
		break;
	case IC_CTRL_READ:
		if (touch_evb_i2c_read(client, value, 1, &buf) < 0) {
			TOUCH_ERR_MSG("IC register read fail\n");
				return -EIO;
			}
			break;
	case IC_CTRL_WRITE:
		if (touch_evb_i2c_write_byte(client, ((value & 0xFFF00) >> 8), (value & 0xFF)) < 0) {
			TOUCH_ERR_MSG("IC register write fail\n");
			return -EIO;
		}
			break;
	case IC_CTRL_RESET_CMD:
		if (unlikely(touch_evb_i2c_write_byte(client, DEVICE_COMMAND_REG, 0x1) < 0)) {
...
struct touch_device_driver synaptics_ts_evb_driver = {
	.probe 	= synaptics_ts_evb_probe,
	.remove	= synaptics_ts_evb_remove,
	.init  	= synaptics_ts_evb_init,
	.data  	= synaptics_ts_evb_get_data,
	.power 	= synaptics_ts_evb_power,
	.fw_upgrade = synaptics_ts_evb_fw_upgrade,
	.ic_ctrl	= synaptics_ts_evb_ic_ctrl,
};
...
{
	touch_evb_driver_register(&synaptics_ts_evb_driver, match_table);
	return;
}

static int __devinit touch_evb_init(void)
{
...
static void __exit touch_evb_exit(void)
{
#ifdef CONFIG_MACH_MSM8926_MADAI
	hw_rev_type hw_rev;
	...
	touch_evb_driver_unregister();
}

module_init(touch_evb_init);
module_exit(touch_evb_exit);
