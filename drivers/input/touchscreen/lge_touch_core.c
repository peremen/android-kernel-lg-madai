...
#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
...
#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
static hw_rev_type g_hw_rev = HW_REV_A;
#endif
...
//u32 touch_debug_mask = DEBUG_BASE_INFO | DEBUG_LPWG_COORDINATES | DEBUG_TRACE | DEBUG_LPWG;
//u32 touch_debug_mask = 0x1FFFF;
...
static int power_control(struct lge_touch_data *ts, int on_off)
{
	...
	// okb_start Lcd_noti to touch_reset
	if(on_off){ 
	#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
		pr_err("[Touch][psh][%s] LCD_EVENT_TP_DISABLE_START ************ \n", __func__);
		touch_notifier_call_chain(LCD_EVENT_TP_DISABLE, NULL);
	#endif
		}
		DO_IF(touch_device_func->power(ts->client, on_off) != 0, error);
	if(on_off){
    #if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
		pr_err("[Touch][psh][%s] LCD_EVENT_TP_ENABLE_START ************* \n", __func__);
		touch_notifier_call_chain(LCD_EVENT_TP_ENABLE, NULL);
	#endif
		}
		//okb_end
...
static void safety_reset(struct lge_touch_data *ts)
{
	...
	DO_SAFE(power_control(ts, POWER_TOUCH_RESET_OFF), error);
	msleep(ts->pdata->role->reset_delay);
	DO_SAFE(power_control(ts, POWER_TOUCH_RESET_ON), error);
	msleep(ts->pdata->role->booting_delay);
#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
	if ( ( hw_rev >= HW_REV_A ) &&  ( hw_rev <= HW_REV_B) ){
				touch_notifier_call_chain(TOUCH_EVENT_RESET_START, NULL);
				pr_err("[psh][%s] notifying LCD_EVENT_ON_START ************************************** \n", __func__);
	}
#endif
...
static void firmware_upgrade_func(struct work_struct *work_upgrade)
{
	...
	if(ERROR == touch_device_func->fw_upgrade(ts->client, &ts->fw_info, ts->pdata->fw)) {
		...
		safety_reset(ts);
			...
			touch_notifier_call_chain(LCD_EVENT_TP_ENABLE, NULL);
	#endif
	//okb_end Lcd_noti to touch_reset
...
static irqreturn_t touch_irq_handler(int irq, void *dev_id)
{
	...
	if (atomic_read(&ts->state.pm_state) == PM_SUSPEND || atomic_read(&ts->state.pm_state) == PM_SUSPEND_IRQ) {
...
static struct attribute *lge_touch_attribute_list[15] = {
...
static struct touch_platform_data* get_dts_data(struct device *dev)
{
	...
	*/
	#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
	if (g_hw_rev >= HW_REV_1_0) {
			of_property_read_string(np, "fw_image_s3320_d0", &p_data->inbuilt_fw_name);
			TOUCH_DEBUG(DEBUG_BASE_INFO, "[%x]fw_image_s3320_d0 : %s",g_hw_rev,p_data->inbuilt_fw_name);
		}
	else if (g_hw_rev == HW_REV_C) {
		of_property_read_string(np, "fw_image_s3320_c0", &p_data->inbuilt_fw_name);
		TOUCH_DEBUG(DEBUG_BASE_INFO, "[%x]fw_image_s3320_c0 : %s",g_hw_rev,p_data->inbuilt_fw_name);
	}
	else if ( ( g_hw_rev == HW_REV_A ) ||  ( g_hw_rev == HW_REV_B) ){
		of_property_read_string(np, "fw_image_s3320_a0", &p_data->inbuilt_fw_name);
		TOUCH_DEBUG(DEBUG_BASE_INFO, "[%x]fw_image_s3320_a0 : %s",g_hw_rev,p_data->inbuilt_fw_name);
	}
	#else
	of_property_read_string(np, "fw_image_s3320_a0", &p_data->inbuilt_fw_name);
	#endif
...
static int touch_pm_suspend_revc(struct device *dev)
{
	struct lge_touch_data *ts = dev_get_drvdata(dev);

	TOUCH_TRACE();
	atomic_set(&ts->state.pm_state, PM_SUSPEND);
	TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : PM_SUSPEND_revc\n", __func__);

	return 0;
}

static int touch_pm_resume_revc(struct device *dev)
{
	struct lge_touch_data *ts = dev_get_drvdata(dev);
	...
	if (atomic_read(&ts->state.pm_state) == PM_SUSPEND_IRQ) {
		...
		TOUCH_DEBUG(DEBUG_BASE_INFO, "resend interrupt_revc");
...
#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
	if ( g_hw_rev >= HW_REV_C ){
		touch_pm_suspend_revc(dev);
	}
#endif
				...
				unsigned long event, void *data)
{
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO,"%s: event = %lu(%d)\n", __func__, event,g_hw_rev);
	...
	const struct i2c_device_id *id)
{
	...
	*/
...
#if defined(CONFIG_INCELL_TOUCHSCREEN_NOTIFIER)
		g_hw_rev = lge_get_board_revno();
		TOUCH_DEBUG(DEBUG_BASE_INFO, "g_hw_rev : %x",g_hw_rev);
#endif	
	...
	if (ts->pdata->reset_pin > 0) {
		DO_SAFE(ret = gpio_request_one(ts->pdata->reset_pin,
			GPIOF_OUT_INIT_HIGH, "touch_reset"), error);
	}

	if (ts->pdata->int_pin > 0) {
		DO_SAFE(ret = gpio_request_one(ts->pdata->int_pin,
			GPIOF_IN, "touch_int"), error);
	}
...
*/
	touch_notifier_call_chain(TOUCH_EVENT_REGISTER_DONE, NULL); //not factory mode!!!!
...
*/
	if ((lge_get_boot_mode() == LGE_BOOT_MODE_QEM_130K) || (lge_get_boot_mode() == LGE_BOOT_MODE_QEM_910K)|| (lge_get_boot_mode() == LGE_BOOT_MODE_PIF_910K) ) {
			TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : No touch panel\n",__func__);
			printk("%s : No touch panel\n",__func__);
			return 0;
		}
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : END(%d)\n",__func__,ret);
	return 0;
