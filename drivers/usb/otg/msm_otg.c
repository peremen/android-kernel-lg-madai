...
{
//reduce usb_psy power_supply_set_supply_type call
#ifdef CONFIG_LGE_PM
	int charger_type;
	if(motg->prev_chg_type == motg->chg_type)
		return 0;
	else
		motg->prev_chg_type = motg->chg_type;
#else
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (motg->pdata->factory_cable_reset) {
			if (lge_pm_get_cable_type() == CABLE_910K && \
				...
				pr_info("[factory] restart to enter download mode\n");
