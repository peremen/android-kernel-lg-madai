...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (get_dload_mode() || in_panic || ((cmd != NULL && cmd[0] != '\0')?strncmp(cmd, "charge_reset", 12):0) || (restart_mode == RESTART_DLOAD)) {
		printk(KERN_NOTICE "Going down by WARM_RESET\n");
		qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
	} else {
		printk(KERN_NOTICE "Going down by HARD_RESET\n");
