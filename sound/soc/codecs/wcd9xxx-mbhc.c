...
{
	printk("sehwan wcd9xxx_check_jack_report \n");
			...
			if (wcd9xxx_is_false_press(mbhc)) {
				...
				if (mbhc->in_swch_irq_handler) {
					...
					wcd9xxx_jack_report(mbhc,
							 ...
							 mbhc->buttons_pressed,
							 mbhc->buttons_pressed);
#ifdef CONFIG_MACH_LGE //2014.05.14.sehwan.lee#lge.com
					msleep(10);
#endif
...
static void wcd9xxx_mbhc_setup(struct wcd9xxx_mbhc *mbhc)
{
	...
	/*                                                    */
	printk("[LGE MBHC] : setup fnt codec_type = %d \n", mbhc->resmgr->codec_type);
