...
#define S(X, Y) ((WCD9XXX_MBHC_CAL_PLUG_TYPE_PTR(tapan_cal)->X) = (Y))
	S(v_no_mic, 30);
	S(v_hs_max, 2500); //S(v_hs_max, 2450);
#undef S
...
/*                                                   */
	...
	if(boost_gpio < 0) {
		dev_err(&pdev->dev, "Looking up %s property in node %s failed %d\n",
			...
			pdev->dev.of_node->full_name, boost_gpio);
	} else {
		ret = gpio_request_one(boost_gpio, GPIOF_OUT_INIT_LOW, "TAPAN_CODEC_BOOSTER");
		printk("[AUDIO_BSP] TAPAN_CODEC_BOOSTER boost_gpio %d ret %d \n", boost_gpio, ret);
		if (ret) {
			dev_err(card->dev,
	...
	printk("[AUDIO_BSP] msm8226_asoc_machine_probe() done~!! \n"); /*                                                            */
	return 0;
